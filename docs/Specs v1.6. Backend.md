...
**2. Connection ID for Client Identification:**

Each WebSocket connection has a cryptographically unique Connection ID:
- Used for lookup in `StopPreview(connectionID)`
- Solves NAT collision issue (multiple clients behind same IP)
- RemoteAddr still logged for debugging purposes
- PreviewID used for resource management (filesystem, maps)
- Separation of concerns: PreviewID = resources, ConnectionID = client identity
...
type Manager struct {
    ...
    connIDToPreview map[string]uint64    // Key: connectionID → PreviewID
...
```go
playlistWaitTimeout = 30 * time.Second   // Wait for playlist.m3u8 creation
previewMaxRuntime   = 30 * time.Second   // Maximum preview duration (auto-stop)
...
**Preview Timeout Rationale:** 30 seconds is sufficient for users to verify source functionality (browser rendering, stream connectivity, file playback). Prevents resource accumulation from forgotten previews. Applies uniformly to all source types (browser_source, ffmpeg_source streams, media files).
...
**8. Playlist Segment Verification:**

The module SHALL verify playlist contains actual segments before declaring preview ready:
...
**Problem Solved:** hls-generator creates `playlist.m3u8` with header metadata before writing video segments. If frontend loads empty playlist, HLS.js throws `levelEmptyError` ("No Segments found in Playlist").

**Solution:** Polling continues until `#EXTINF:` tag detected (indicates at least one segment present). Prevents premature `previewReady` notification.

**Typical timeline for browser_source:**
- t=0s: Process spawned
- t=5-8s: CEF initialized, playlist.m3u8 created (header only)
- t=9-10s: First segment written, #EXTINF: appears
- t=10s: previewReady sent to frontend
...
type StartPreviewRequest struct {
    ...
    OnStopped func(reason string)   // Called when preview is auto-stopped (timeout, etc)
}
```

**Session struct (internal):**
...
2. **processPreview goroutine:** Spawns hls-generator, polls for playlist with segment verification
3. **OnReady callback:** Invoked with HLS URL when playlist contains segments (#EXTINF:)
4. **Timeout timer scheduled:** 30-second auto-stop timer created after OnReady
5. **OnError callback:** Invoked on failure (spawn error, timeout, etc)
6. **OnStopped callback:** Invoked before automatic timeout cleanup (with 100ms delay for WebSocket delivery)
7. **StopPreview:** Cancels timeout timer, kills process, cleans filesystem, removes from maps
8. **Auto-cleanup:** On WebSocket disconnect

**Timeout Flow:**
- After `OnReady`, `time.AfterFunc(30s)` scheduled
- Timer stored in `Session.TimeoutTimer` for cancellation
- On timeout expiry: `OnStopped("Preview automatically stopped after 30 seconds")` → 100ms delay → `StopPreview(connectionID)`
- On manual stop: `TimeoutTimer.Stop()` → prevents timeout firing
...
- Playlist generation timeout (30s, no playlist created or no segments)
...
1. **Called from:** `WebServer.cleanup()` during server shutdown (added in v1.6)
2. **Collect active sessions:** Copy all PreviewIDs while holding read lock
3. **Parallel termination:** Launch goroutine per preview with WaitGroup
   - Send SIGTERM (graceful shutdown)
   - Wait 5 seconds grace period
   - Send SIGKILL if still running
4. **Wait for completion:** WaitGroup.Wait() ensures all processes terminated
5. **Cleanup filesystem:** Remove entire HLS base directory
6. **Clear state:** Nil out maps (idempotent via sync.Once)
...
The SourcePreview module SHALL support `browser_source` input types via hls-generator with CEF (Chromium Embedded Framework) rendering:

- **Rendering:** hls-generator uses CEF to render web pages as video
- **Initialization time:** ~5-10 seconds for CEF to initialize and create first segments
- **Loading feedback:** Frontend displays "Loading browser engine (5-10 seconds)..." message
- **Segment verification:** Essential for browser sources due to long CEF startup time
- **Timeout:** 30 seconds applies (sufficient for CEF initialization + rendering)

**Typical browser_source timeline:**
- t=0-5s: CEF initialization
- t=5-8s: Page rendering, playlist.m3u8 created (header only)
- t=8-10s: First segments generated (#EXTINF: appears)
- t=10s: previewReady sent, frontend starts playback
- t=40s: Automatic timeout, preview stopped

All source types (browser_source, ffmpeg_source, media_source, vlc_source, image_source) use unified HLS workflow.
